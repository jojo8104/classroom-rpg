import type { Lesson, SessionType, Student, StudentLessonState, Teacher } from '../domain.js';
import { bounded } from '../systems/LearningMemory.js';
import { sessionRules } from '../data/sessionRules.js';

export function exerciseFit(mastery: number, difficulty: number, pedagogy: number) {
...
export function sessionWork(type: SessionType, student: Student, state: StudentLessonState, lesson: Lesson, teacher: Teacher, difficulty = 50) {
  const mastery = student.lessonMastery?.[lesson.id] ?? 0;
  if (type === 'lecture') return { lesson, multiplier: 1, morale: 0 };
  if (type === 'assessment') return { lesson: { ...lesson, pressure: sessionRules.assessmentPressure, complexity: 30 },
    multiplier: (0.05 + mastery / 100 * 2) * (0.3 + student.discipline / 100 * 0.7) * (0.25 + state.concentration / 100 * 0.75), morale: 0 };
  if (type === 'revision') return { lesson: { ...lesson, pressure: lesson.pressure * 0.25, complexity: lesson.complexity * 0.5 }, multiplier: 0.8, morale: 0 };
