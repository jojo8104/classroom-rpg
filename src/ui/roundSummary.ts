...
export function summarizeRound(before: readonly StudentLessonState[], after: readonly StudentLessonState[],
  ...
  return after.map(state => {
    ...
    if (state.morale <= summaryThresholds.lowMorale) alerts.push('Moral faible');
    if (concentration <= -summaryThresholds.largeLoss) alerts.push('Forte perte de concentration');
    if (!completed && progress < summaryThresholds.lowProgress) alerts.push('Peu de progression');
    const received = { support: 0, protection: 0, combo: 0, disruptionDamage: 0 };
    for (const event of events) {
      if ('targetId' in event && event.targetId === state.studentId) {
        if (event.type === 'EFFECT_APPLIED' && event.amount > 0) received.support++;
        if (event.type === 'REACTION_TRIGGERED' && event.before !== event.after) {
          if (event.effect === 'REDUCE_PRESSURE') received.protection++; else received.support++;
        }
        if (event.type === 'COMBINED_ATTACK_RESOLVED' && event.appliedProgress > 0) received.combo++;
        if (event.type === 'DISRUPTION_RESOLVED') received.disruptionDamage = roundValue(received.disruptionDamage + event.damage);
      }
    }
