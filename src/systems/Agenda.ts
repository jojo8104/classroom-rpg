...
export class Agenda {
  ...
  get currentDay() {
    ...
    for (const week of weeks) {
      ...
      if (index >= 0) return (week.number - 1) * weekLength + index;
    }
  ...
  private validateSave(s: AgendaSave) {
    ...
    for (const session of s.sessions) {
      ...
      if (session.sessionType && session.sessionType !== session.type) throw new Error('Types incohérents.');
      this.validateDifficulty(session.difficulty);
      const index = slots.findIndex(slot => slot.id === session.slotId);
      if (typeof session.id !== 'string' || !session.id.trim() || ids.has(session.id) || occupied.has(session.slotId + (session.type === 'homework' ? '/homework' : '/class')) || index < 0 || !['planned', 'completed'].includes(session.status) || (session.status === 'completed' ? (session.type === 'homework' ? index > s.cursor : index >= s.cursor) : index < s.cursor)) throw new Error('Séance sauvegardée invalide.');
      ids.add(session.id); occupied.add(session.slotId + (session.type === 'homework' ? '/homework' : '/class'));
    }
    ...
    if ([...preparation.values()].some(minutes => minutes > sessionRules.dailyPreparationMinutes)) throw new Error('Temps de préparation invalide.');
  }
  ...
  private validateDifficulty(difficulty?: number) {
    if (difficulty !== undefined && (!Number.isFinite(difficulty) || difficulty < 0 || difficulty > 100)) throw new Error('Difficulté attendue entre 0 et 100.');
  }
  private persist() {
  ...
  place(slotId: string, lessonId: string, type: string, sessionId?: string, difficulty?: number) {
    ...
    if (index < this.state.cursor || index < 0 || this.state.sessions.some(s => s.slotId === slotId && s.id !== sessionId && (s.type === 'homework') === (type === 'homework'))) throw new Error('Créneau passé, inexistant ou déjà occupé.');
    if (type === 'homework') {
      ...
      if ((count + 1) * sessionRules.homeworkMinutes > sessionRules.dailyPreparationMinutes) throw new Error('Temps libre de préparation épuisé pour cette journée.');
    }
    if (existing) { Object.assign(existing, { slotId, lessonId, type, sessionType: type }); if (difficulty === undefined) delete existing.difficulty; else existing.difficulty = difficulty; }
    else {
      let id: string;
      do { id = `session-${this.state.nextId++}`; } while (this.state.sessions.some(s => s.id === id));
      this.state.sessions.push({ id, slotId, lessonId, type, sessionType: type as import('../domain.js').SessionType, ...(difficulty === undefined ? {} : { difficulty }), status: 'planned' });
    }
    this.persist();
  }
  remove(id: string) {
  ...
  consume(id: string) {
    ...
    this.state.sessions.find(s => s.id === id)!.status = 'completed'; if (session.type !== 'homework') this.state.cursor++; this.persist();
  }
  skip() {
    if (!this.currentSlot || this.state.sessions.some(s => s.slotId === this.currentSlot!.id && s.status === 'planned')) throw new Error('Seul un créneau courant vide peut être laissé libre.');
    this.state.cursor++; this.persist();
  }
  indicators(students: readonly Student[], deadlineSlotId?: string, pedagogy = 0) {
