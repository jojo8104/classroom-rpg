import type { Concept, Lesson, Student, Teacher } from '../domain.js';
...
export function initializeConcepts(student: Student) {
...
export function hasConcept(student: Student, conceptId: string): boolean {
...
export class Concepts {
  ...
  getConcept(id: string) { return this.catalog.find(c => c.id === id); }
  hasConcept = hasConcept;
  getConceptsByTag(student: Student, tag: string) { return this.getConceptsByTags(student, [tag]); }
  /** Tous les tags demandés doivent être présents ; [] renvoie tous les acquis. */
  getConceptsByTags(student: Student, tags: readonly string[]) {
    return this.catalog.filter(c => hasConcept(student, c.id) && tags.every(tag => c.tags.includes(tag)));
  }
  learningMultiplier(student: Student, tags: readonly string[]): number {
  ...
  attemptDiscoveries(context: Omit<DiscoveryContext, 'conceptId' | 'baseRate'>, xpGained: number, random: { next(): number }): GameEventPayload[] {
    ...
    for (const entry of context.lesson.conceptIds.map(conceptId => ({ conceptId, baseRate: context.lesson.conceptPool?.find(e => e.conceptId === conceptId)?.baseRate ?? this.getConcept(conceptId)?.baseRate ?? 0 }))) {
      ...
      } else state.progress[entry.conceptId] = clamp((state.progress[entry.conceptId] ?? 0) + conceptRules.failureProgress);
    }
    return events;
  }
}
