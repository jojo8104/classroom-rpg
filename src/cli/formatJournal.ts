...
export function formatJournal(scenario: PrototypeScenario, result: SimulationResult): string {
  ...
  for (const event of result.events) {
    switch (event.type) {
      case 'LEARNING_OBSERVED': lines.push(`${name(event.studentId)} : ${event.label} · ${event.feedback}`); break;
      case 'concept_reactivated': lines.push(`${name(event.studentId)} réactive ${event.conceptId}`); break;
