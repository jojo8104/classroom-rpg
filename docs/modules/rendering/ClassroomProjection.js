...
export class ClassroomProjection {
    ...
    columns;
    config;
    constructor(rows = 5, columns = 5, config = CLASSROOM_PERSPECTIVE) {
        this.rows = rows;
        this.columns = columns;
        this.config = config;
    }
    project(row, column) {
        if (!Number.isInteger(row) || !Number.isInteger(column) || row < 0 || row >= this.rows || column < 0 || column >= this.columns)
            throw new RangeError('Place hors de la classe');
        const c = this.config;
