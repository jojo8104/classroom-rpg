...
export function* resolveWorkTurn(context) {
    ...
    if (state.progress >= lesson.requiredProgress)
        return;
    ...
    if (combination) {
        ...
        if (combination.abilityId)
            events.push({ type: 'ABILITY_USED', studentId: partner.id, targetId: student.id, abilityId: combination.abilityId, effective: state.progress > before });
    }
    else
    ...
    if (state.concentration <= 0 || critical || state.progress >= lesson.requiredProgress)
        return;
