...
export function runProgressionStudy(seed = 12345, lessons = 20) {
    ...
    const history = [];
    const used = Object.fromEntries(abilities.map(a => [a.id, 0]));
    for (let lesson = 0; lesson < lessons; lesson++) {
        ...
        for (const student of result.nextLessonStudents)
            if (student.progression.level >= progressionRules.specializationLevel && !student.progression.specialization) {
        ...
        for (const event of result.events)
            if (event.type === 'ABILITY_USED' && event.effective)
                used[event.abilityId] = (used[event.abilityId] ?? 0) + 1;
