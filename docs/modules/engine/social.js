...
export const personalityTraits = ['altruistic', 'competitive', 'persistent', 'impulsive', 'calm', 'sociable'];
...
export function trait(student, key) { return student.personality?.[key] ?? 0; }
export function validateSocial(students, relations) {
    const errors = [], ids = new Set(students.map(s => s.id)), pairs = new Set();
    for (const student of students)
        for (const [key, value] of Object.entries(student.personality ?? {})) {
            if (!personalityTraits.includes(key) || !Number.isFinite(value) || value < 0 || value > 1)
                errors.push('Personnalité : trait ou intensité invalide.');
        }
    for (const link of relations?.links ?? []) {
        const key = JSON.stringify([link.from, link.to]);
        if (!ids.has(link.from) || !ids.has(link.to) || link.from === link.to || pairs.has(key))
            errors.push('Relation orientée : élèves inconnus, identiques ou lien dupliqué.');
        if (!Number.isFinite(link.score) || link.score < socialRules.min || link.score > socialRules.max)
            errors.push('Relation orientée : score invalide.');
        pairs.add(key);
    }
    return errors;
}
export function clampRelation(score) {
    if (!Number.isFinite(score))
        throw new Error('Score relationnel invalide.');
    return roundValue(Math.max(socialRules.min, Math.min(socialRules.max, score)));
}
export function getRelation(relations, from, to) {
    return relations.links.find(link => link.from === from && link.to === to)?.score ?? 0;
}
export function modifyRelation(relations, from, to, delta) {
    if (from === to)
        throw new Error('Une relation exige deux élèves distincts.');
    ...
    if (link)
        ...
        relations.links.push({ from, to, score: after });
    return { before, after };
}
export function relationZone(score) {
...
export function migrateRelations(legacy) {
    return { links: legacy.flatMap(({ studentIds: [a, b], value }) => [{ from: a, to: b, score: value }, { from: b, to: a, score: value }]) };
}
export function createInitialRelations(students, seed) {
    const random = new SeededRandom(seed), links = [];
...
export function difficultyLoss(student, loss) {
    return roundValue(loss * (1 - socialRules.persistenceLoss * trait(student, 'persistent') - socialRules.calmLoss * trait(student, 'calm')));
}
export function relationThreshold(student, minimum, synergy = false) {
    return synergy ? minimum : Math.max(0, minimum - socialRules.altruisticThreshold * trait(student, 'altruistic'));
}
export function behaviorCandidate(student, target, action, relation, baseWeight = socialRules.base, masteryValid = true) {
...
export function weightedChoice(candidates, random) {
    if (candidates.some(c => !Number.isFinite(c.finalWeight) || c.finalWeight < 0))
    ...
    for (const candidate of candidates) {
        draw -= candidate.finalWeight;
        if (draw < 0)
            return candidate;
    }
    return candidates.filter(c => c.finalWeight > 0).at(-1);
}
export function chooseBehavior(candidates, random, events) {
    for (const candidate of candidates)
        events.push({ type: 'BEHAVIOR_CANDIDATE_CREATED', ...candidate });
    const chosen = weightedChoice(candidates, random);
    if (chosen)
        events.push({ type: 'BEHAVIOR_SELECTED', ...chosen });
    return chosen;
}
export function evolveRelation(relations, owner, otherId, delta, reason, events) {
    ...
    if (result.before !== result.after)
        events.push({ type: 'RELATION_CHANGED', from: owner.id, to: otherId, ...result, reason });
}
