    ...
    for (const event of events) {
        if (event.type === 'ROUND_STARTED') {
            ...
            if (event.reason !== 'support')
                feedback(event.studentId, event.reason === 'pressure' ? `−${Math.round((event.before - event.after) * 100) / 100} HP` : `+${Math.round((event.after - event.before) * 100) / 100} HP · reprise au prochain tour`);
        }
