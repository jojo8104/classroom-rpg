...
catch { /* Le jeu reste disponible sans stockage. */ }
...
for (const direction of [-1, 1])
    el(direction < 0 ? 'scroll-left' : 'scroll-right').onclick = () => el('seat-viewport').scrollBy({ left: direction * 240, behavior: 'smooth' });
...
lessonPanel.id = 'lesson-results';
lessonPanel.className = 'round-result';
lessonPanel.hidden = true;
lessonPanel.setAttribute('aria-label', 'Bilan de la leçon');
el('round-result').after(lessonPanel);
const studentName = (id) => scenario.students.find(s => s.id === id)?.name ?? 'Professeur';
function renderStudents() {
    ...
    if (reactionFeedback.has(selected)) {
        ...
        last.textContent = reactionFeedback.get(selected);
        ...
        for (const ability of abilities) {
            ...
            switch (ability.effect) {
                case 'REDUCE_PRESSURE':
                    description.textContent = `Protège un voisin contre la riposte · relation ≥ ${ability.minRelation}/100.`;
                    break;
                case 'REDUCE_COMPLEXITY':
                    description.textContent = `Affaiblit la complexité pour l’attaque d’un voisin · relation ≥ ${ability.minRelation}/100.`;
                    break;
                case 'COMBINED_ATTACK':
                    description.textContent = `Attaque combinée avec un voisin · relation ≥ ${ability.minRelation}/100. Remplace l’affaiblissement simple ; le camarade reçoit la progression.`;
                    break;
                case 'APPLY_TEMPORARY_EFFECT':
                    description.textContent = `${statNames[ability.stat]} + : ${timing}, pendant ${ability.durationInRounds} rounds (round actuel inclus) · relation ≥ ${ability.minRelation}/100.`;
                    break;
            }
    ...
    if (state.effects.length) {
        ...
        heading.textContent = 'Bonus actifs';
    ...
    if (matrixOpen) {
        ...
        el('student-detail').append(matrix);
    }
    else {
        const matrix = document.createElement('details');
        matrix.id = 'relations-debug';
        ...
        summary.textContent = 'Relations orientées · matrice de debug';
        matrix.append(summary);
        matrix.addEventListener('toggle', () => { if (matrix.open)
            renderStudents(); }, { once: true });
        el('student-detail').append(matrix);
    }
    ...
    if (p) {
        ...
        const heading = document.createElement('h3');
        heading.textContent = 'Progression RPG';
        ...
        trend.textContent = 'Tendance : ' + trends.map(t => `${t.name} ${t.percent} %`).join(' · ');
...
const actionSelect = el('teacher-action');
const targetSelect = el('teacher-target');
...
function renderTeacher() {
    if (gameState === GAME_STATE.CLASS_PREPARATION) {
        el('teacher-controls').hidden = true;
        return;
    }
    ...
    el('teacher-patience').max = shownTeacher.maxPatience || 1;
    el('teacher-patience').value = shownTeacher.patience;
    el('teacher-controls').hidden = simulation.state !== 'TEACHER_INTERVENTION';
    if (!simulation.hasNextRound)
        actionSelect.value = 'PASS';
    ...
    el('teacher-confirm').disabled = busy || !preview.available;
...
function syncControls() {
    ...
    if (gameState === GAME_STATE.CLASS_PREPARATION) {
        ...
        seed.disabled = false;
        restart.querySelector('button').disabled = false;
    ...
    renderTeacher();
    if (busy) {
        advance.textContent = 'Résolution en cours…';
        return;
    }
    if (simulation.state === 'TEACHER_INTERVENTION') {
        ...
        if (activeSessionId) {
            ...
            preparation.save();
            agenda.consume(activeSessionId);
...
function renderRoundResult(before, events) {
    ...
    const nameButton = (id) => {
        const button = document.createElement('button');
        button.type = 'button';
        button.textContent = studentName(id);
        button.className = 'summary-student';
        button.setAttribute('aria-label', `Sélectionner ${studentName(id)} pour l’intervention`);
        button.addEventListener('click', () => selectStudent(id));
        return button;
    };
    ...
    for (const row of concerns) {
        ...
        item.append(nameButton(row.studentId), ` : ${row.alerts.join(' · ')}.`);
        el('round-attention').append(item);
    }
    if (!concerns.length)
        el('round-attention').textContent = 'Aucun point d’attention selon les seuils du bilan.';
    ...
    for (const state of states) {
        ...
        nameCell.append(nameButton(state.studentId));
        row.append(nameCell);
...
el('teacher-form').addEventListener('submit', async (event) => {
    event.preventDefault();
    if (gameState === GAME_STATE.CLASS_PREPARATION || busy || simulation.state !== 'TEACHER_INTERVENTION')
        return;
    const kind = actionSelect.value;
    const action = kind === 'PASS' || kind === 'BREAK' ? { kind } : { kind, targetId: selected };
    busy = true;
    syncControls();
    try {
        await showEvents(simulation.applyTeacherAction(action));
    }
    catch (error) {
        note(error instanceof Error ? error.message : 'Intervention impossible.');
    }
    finally {
        busy = false;
        syncControls();
...
function feedback(studentId, text) {
    el(studentId).querySelector('.seat-feedback').textContent = text;
}
async function showEvents(events) {
    const playback = simulation.individual && speed.value !== 'instant' ? 'compact' : speed.value;
    if (playback === 'instant')
        renderer?.reset();
    for (const event of visualEvents(events, playback)) {
        ...
        if (event.type === 'LESSON_RESULTS') {
            ...
            title.textContent = 'Bilan de la leçon';
            panel.append(title);
            for (const result of event.results) {
                ...
                details.textContent = Object.entries(p.rewards).map(([k, v]) => `${rewardLabels[k]} +${v}`).join(' · ');
                row.append(document.createElement('br'), details);
                if (p.unlockedAbilities.length)
                    row.append(document.createElement('br'), 'Compétences apprises : ' + p.unlockedAbilities.map(id => abilityCatalog.find(a => a.id === id).name).join(', '));
                const discoveries = events.filter(e => e.type === 'concept_discovered' && e.studentId === result.studentId);
                for (const discovery of discoveries)
                    if (discovery.type === 'concept_discovered')
                        row.append(document.createElement('br'), 'Concept découvert : ' + (scenario.concepts.find(c => c.id === discovery.conceptId)?.name ?? discovery.conceptId));
                panel.append(row);
            }
            renderStudents();
        }
            ...
            shownTeacher.patience = event.after;
            renderTeacher();
        }
            ...
            note('Relation ' + studentName(event.from) + ' → ' + studentName(event.to) + ' : ' + event.before + ' → ' + event.after);
            renderStudents();
        }
            ...
            state.effects.push(structuredClone(event.effect));
            renderStudents();
            if (event.refreshed)
                note(`${studentName(event.effect.targetId)} : bonus de ${statNames[event.effect.stat].toLowerCase()} renouvelé pour ${event.effect.remainingRounds} rounds, round actuel inclus.`);
        }
            ...
            if (effect)
                effect.remainingRounds = event.effect.remainingRounds;
            renderStudents();
        }
            ...
            if (event.effect === 'REDUCE_COMPLEXITY')
                el('status').textContent = `${studentName(event.sourceId)} affaiblit la leçon pour ${studentName(event.targetId)} : complexité ${event.before} → ${event.after}, pour cette attaque.`;
            ...
            state.missedRounds = event.missedRounds;
            renderStudents();
        }
        else if (event.type === 'CONCENTRATION_CHANGED') {
            shownStates.get(event.studentId).concentration = event.after;
            renderStudents();
            if (event.reason === 'effort')
                note(`${studentName(event.studentId)} : effort de travail, −${Math.round((event.before - event.after) * 100) / 100} concentration.`);
            if (event.reason !== 'support')
                feedback(event.studentId, event.after < event.before ? `−${Math.round((event.before - event.after) * 100) / 100} HP` : `+${Math.round((event.after - event.before) * 100) / 100} HP · reprise au prochain tour`);
        }
        else if (event.type === 'MORALE_CHANGED') {
            shownStates.get(event.studentId).morale = event.after;
            renderStudents();
            ...
            feedback(event.studentId, 'Décroche');
            note(`${studentName(event.studentId)} décroche : concentration épuisée.`);
        }
        ...
        if (playback === 'compact' && !['ROUND_ENDED', 'LEARNING_OBSERVED', 'concept_reactivated'].includes(event.type))
    ...
    shownTeacher = simulation.teacherState;
    renderStudents();
    if (playback === 'instant')
        console.info('Simulation instantanée', simulation.debugMetrics);
    clearActionFeedback();
}
async function advanceLesson() {
    if (gameState === GAME_STATE.CLASS_PREPARATION || busy || simulation.state !== 'ROUND_READY')
        return;
    busy = true;
    syncControls();
    try {
        if (simulation.state === 'ROUND_READY') {
            ...
            completedRounds++;
            renderRoundResult(before, events);
...
advance.addEventListener('click', () => { if (gameState === GAME_STATE.CLASS_PREPARATION)
    startLesson();
else
    void advanceLesson(); });
function startLesson() {
    if (!restart.reportValidity())
        return;
    try {
        const session = agenda.nextSession();
        const selectedScenario = selectLessonActivity(scenario, curriculum, { lessonId: session.lessonId, teachingMode: session.type, sessionType: session.type, ...(session.difficulty === undefined ? {} : { difficulty: session.difficulty }), day: agenda.currentDay });
        ...
        option.textContent = getTeachingMode(session.type).name;
    ...
    if (selected)
        renderStudents();
    syncControls();
    if (simulation.individual)
        void runIndividualSession();
}
async function runIndividualSession() {
    ...
    try {
        while (simulation.state !== 'LESSON_FINISHED') {
            const before = simulation.studentStates;
            await showEvents(simulation.resolveRound());
            completedRounds++;
...
for (const id of scenario.lesson.availableTeachingModes ?? ['lecture']) {
    ...
    option.textContent = mode.name + (option.disabled ? ' (différé, à venir)' : '');
    modeSelect.append(option);
}
const curriculumPath = document.createElement('p');
curriculumPath.className = 'lesson-stats';
curriculumPath.textContent = `${scenario.program?.name ?? ''} / ${scenario.subject.name} / ${scenario.topic.name} / ${scenario.lesson.name}`;
...
const agendaRoot = document.createElement('section');
agendaRoot.id = 'agenda';
