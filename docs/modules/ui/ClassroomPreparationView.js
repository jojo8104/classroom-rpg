...
export class ClassroomPreparationView {
    ...
    constructor(preparation, grid, detail) {
        ...
        for (const [label, action] of [['Réinitialiser le placement', () => preparation.reset()], ['Sauvegarder le placement', () => preparation.save()]]) {
            const button = document.createElement('button');
            button.type = 'button';
            button.textContent = label;
            button.onclick = () => { action(); this.render(); };
            this.tools.append(button);
        }
        this.message.setAttribute('role', 'status');
        this.tools.append(this.message);
        this.grid.before(this.tools);
        this.preview.id = 'placement-preview';
        this.preview.setAttribute('aria-live', 'polite');
        this.preview.setAttribute('aria-atomic', 'true');
        ...
        grid.addEventListener('pointerdown', event => {
            ...
            if (preparation.system.getStudentSeat(id)?.locked)
                return;
        ...
        document.addEventListener('pointermove', event => {
            ...
            if (seatId !== this.hover) {
                this.hover = seatId;
                this.grid.querySelectorAll('[data-seat]').forEach(el => el.classList.toggle('drop-target', el.dataset.seat === seatId));
                if (seatId)
                    this.showPreview(seatId);
            }
    ...
    move(seatId) {
        ...
        this.moving = false;
        this.render();
        this.grid.querySelector(`[data-student="${this.selected}"]`)?.focus();
    }
    render() {
        const { scenario, system } = this.preparation, layout = system.currentLayout;
        this.grid.replaceChildren();
        this.grid.style.gridTemplateColumns = `repeat(${layout.columns}, minmax(0, 1fr))`;
        ...
        for (const seat of layout.seats) {
            ...
            cell.className = 'preparation-seat';
            cell.dataset.seat = seat.id;
            ...
            if (student) {
                ...
                paragraph(button, 'Place vide', 'strong');
                button.setAttribute('aria-label', `Place vide ${label}`);
            }
            button.onclick = () => {
                if (this.moving) {
                    this.move(seat.id);
                ...
                if (student) {
                    ...
                    this.render();
                    this.grid.querySelector(`[data-student="${student.id}"]`)?.focus();
                }
            };
            button.onpointerenter = () => { if (this.moving)
                this.showPreview(seat.id); };
            button.onfocus = () => { if (this.moving)
                this.showPreview(seat.id); };
            cell.append(button);
    ...
    renderDetail() {
        ...
        if (!student) {
            this.detail.textContent = 'Aucun élève présent.';
            return;
        }
        renderStudentCard(this.detail, student, this.preparation.scenario);
        const move = document.createElement('button');
        move.type = 'button';
        move.textContent = this.moving ? 'Annuler le déplacement' : 'Déplacer cet élève';
        move.disabled = this.preparation.system.getStudentSeat(student.id)?.locked ?? true;
        move.onclick = () => { this.moving = !this.moving; this.renderDetail(); if (this.moving) {
            this.message.textContent = 'Choisissez la place de destination. Échap pour annuler.';
            this.grid.querySelector('.seat')?.focus();
        } };
    ...
    showPreview(seatId) {
        ...
        if (!result.neighbors.length)
            paragraph(this.preview, 'Aucun voisin direct.');
        for (const neighbor of result.neighbors) {
            ...
            if (neighbor.useful)
                paragraph(this.preview, 'Soutien pédagogique potentiel — à mettre en balance avec la relation.');
        }
        const synergies = [...new Set(result.neighbors.flatMap(n => n.synergies.map(s => s.name)))];
        if (synergies.length)
            paragraph(this.preview, 'Compétences de voisinage : ' + synergies.join(', '));
