export function createTeacherRules() {
...
export function validateTeacherRules(rules) {
    if (!Number.isFinite(rules.statReference) || rules.statReference <= 0 ||
        !Number.isFinite(rules.disruptionPower) || rules.disruptionPower < 0 || rules.disruptionPower > 100)
        throw new Error('Règles du professeur invalides.');
    ...
    for (const kind of Object.keys(expected)) {
        ...
        if (!definition || definition.target !== expected[kind].target || definition.stat !== expected[kind].stat ||
            ...
            kind !== 'PASS' && definition.cost === 0)
            throw new Error('Intervention configurée invalide.');
    }
    const pass = rules.actions.PASS;
    if (pass.cost !== 0 || pass.power !== 0 || pass.duration !== 0)
        throw new Error('PASS doit rester gratuit et sans effet.');
}
