import { CLASSROOM_CONFIG } from './classroomConfig.js';
...
export function createFullClassPrototype(config = CLASSROOM_CONFIG) {
    if (!Number.isSafeInteger(config.maxStudents) || config.maxStudents < 0 || config.maxStudents > config.rows * config.columns)
        throw new Error('Effectif incompatible avec la grille.');
    const scenario = createPreparationPrototype(), random = new SeededRandom(701);
    const roles = scenario.archetypes.map(a => a.id);
    scenario.students = Array.from({ length: config.maxStudents }, (_, i) => {
        const archetypeId = roles[i % roles.length];
        const struggling = i % 4 === 0, turbulent = i % 6 === 3;
        const personality = createPersonality(710 + i);
        if (archetypeId === 'support')
            personality.altruistic = 0.8;
        if (turbulent) {
            personality.impulsive = 0.9;
            personality.calm = 0.1;
        }
    ...
    if (first && last && first.id !== last.id && !scenario.classRelations.links.some(l => l.from === first.id && l.to === last.id))
        scenario.classRelations.links.push({ from: first.id, to: last.id, score: 72 });
    return scenario;
}
