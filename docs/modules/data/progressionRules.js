...
export const rewardLabels = { participation: 'Participation à la leçon', offense: 'Contribution offensive', concentration: 'Soutien de concentration', morale: 'Soutien moral', pedagogy: 'Aide pédagogique', defense: 'Protection utile', combo: 'Attaque combinée', perseverance: 'Difficulté surmontée', personal: 'Progression personnelle' };
export function getRequiredXpForLevel(level) {
    if (!Number.isSafeInteger(level) || level < 1 || level > progressionRules.thresholds.length)
        throw new Error('Niveau invalide.');
    return progressionRules.thresholds[level - 1];
}
export function levelForXp(xp) {
    if (!Number.isSafeInteger(xp) || xp < 0)
        throw new Error('XP invalide.');
