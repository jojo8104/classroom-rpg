export function validateCalendarConfig(c) {
    const integer = (n, min, max) => Number.isSafeInteger(n) && n >= min && n <= max;
    const unique = (ids) => ids.every(id => typeof id === 'string' && id.trim()) && new Set(ids).size === ids.length;
    if (!c.id?.trim() || !c.name?.trim() || !integer(c.teachingWeeks, 1, 200) || !integer(c.slotsPerDay, 1, 24)
        ...
        || !Number.isFinite(c.masteryThreshold) || c.masteryThreshold < 0 || c.masteryThreshold > 100)
        throw new Error('Configuration du calendrier invalide : vérifiez semaines, périodes, jours et créneaux.');
    const weeks = c.periods.reduce((sum, p) => sum + p.teachingWeeks + p.holidayWeeksAfter, 0);
    if (!unique(c.specialDays.map(d => `${d.week}/${d.dayId}`)) || c.specialDays.some(d => !integer(d.week, 1, weeks) || !c.days.some(day => day.id === d.dayId) || !['school', 'holiday', 'event'].includes(d.status) || typeof d.label !== 'string' || (d.slots !== undefined && (!integer(d.slots, 0, 24) || d.status !== 'school'))))
        throw new Error('Jours particuliers invalides.');
    for (const d of c.specialDays) {
        let end = 0;
        const period = c.periods.find(p => { end += p.teachingWeeks + p.holidayWeeksAfter; return d.week <= end; });
        if (d.status === 'school' && d.week > end - period.holidayWeeksAfter)
            throw new Error('Une semaine de vacances ne peut pas contenir un jour scolaire.');
    }
}
export function generateSchoolCalendar(config) {
            ...
            weeks: Array.from({ length: period.teachingWeeks + period.holidayWeeksAfter }, (_, i) => {
                ...
                return { id, number: week, periodId: period.id, teaching, days: config.days.map(day => {
                        ...
                        return { id: dayId, name: day.name, status, label: special?.label ?? (teaching ? (status === 'school' ? 'Enseignement' : 'Jour non travaillé') : 'Vacances'),
                            slots: Array.from({ length: status === 'school' ? (special?.slots ?? config.slotsPerDay) : 0 }, (_, index) => ({ id: `${dayId}/slot-${index + 1}`, index, scope: 'school' })) };
                    }) };
            }) })) };
}
