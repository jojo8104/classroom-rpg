...
export class Agenda {
    ...
    validateSave(s) {
        ...
        for (const session of s.sessions) {
            ...
            if (typeof session.id !== 'string' || !session.id.trim() || ids.has(session.id) || occupied.has(session.slotId) || index < 0 || !['planned', 'completed'].includes(session.status) || (session.status === 'completed' ? index >= s.cursor : index < s.cursor))
    ...
    place(slotId, lessonId, type, sessionId) {
        ...
        if (index < this.state.cursor || index < 0 || this.state.sessions.some(s => s.slotId === slotId && s.id !== sessionId))
            throw new Error('Créneau passé, inexistant ou déjà occupé.');
        if (existing)
            Object.assign(existing, { slotId, lessonId, type });
        else {
            let id;
            do {
    ...
    nextSession() {
        ...
        if (getTeachingMode(session.type).resolution !== 'classroom')
            throw new Error('Les devoirs sont planifiables ; leur résolution différée arrivera ultérieurement. Modifiez ou déplacez cette séance pour poursuivre en classe.');
    ...
    skip() {
        if (!this.currentSlot || this.state.sessions.some(s => s.slotId === this.currentSlot.id))
            throw new Error('Seul un créneau courant vide peut être laissé libre.');
        this.state.cursor++;
        this.persist();
    }
    indicators(students, deadlineSlotId) {
